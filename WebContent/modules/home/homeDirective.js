define(['crapModule'], function (app){
	...
	app.lazy.directive('ngAutocomplete', ['$timeout', '$filter', 'ngAutocompleteService', function($timeout, $filter, ngAutocompleteService) {
		'use strict';
		...
		return {
			...
			'<input type="text" ng-model="keyword" class="ng-autocomplete-input form-control" style="width: 20%;" placeholder="{{ placeholder }}" ng-change="getResults()" ng-blur="blurHandler()" ng-focus="currentIndex = 0" autocorrect="off" autocomplete="off">' +
			'<div id="dropdown" class="ng-autocomplete-holder" ng-show="showResults">' +
	...
	app.lazy.service('ngAutocompleteService', ['$http', function($http) {
		var self = this;
	...
	app.lazy.directive('ngBlur', ['$parse', function($parse) {
